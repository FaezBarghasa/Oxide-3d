...
pub fn render_dcc_workspace(app: &OxideApp) -> Element<'_, OxideUiMessage> {
    ...
    let menu_dropdown: Option<Element<'_, OxideUiMessage>> = app.dcc_menu.active_menu.map(|cat| {
        ...
        let items_col =
            items
                .into_iter()
                .take(15)
                .fold(column![].spacing(2), |col, item: DccMenuItemDef| {
    ...
    let toolbar_row = row![
        ...
        text("│").size(10),
        button(
            text(if app.dcc_toolbar.snaps.grid_snap {
                "Grid [ON]"
            } else {
                "Grid [OFF]"
            })
        ...
        .padding([2, 5]),
        button(
            text(if app.dcc_toolbar.snaps.angle_snap {
                "Angle [ON]"
            } else {
                "Angle [OFF]"
            })
    ...
    let ribbon_container =
        container(column![ribbon_tab_buttons, ribbon_subobjects].spacing(3)).padding(3);
    ...
    let panel_content =
        match app.command_panel.active_tab {
            CommandPanelTab::Create => column![
                text("Standard Primitives").size(12),
                row![
                    button(text("Box").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Box".to_string())),
                    button(text("Sphere").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Sphere".to_string())),
                ]
                .spacing(4),
                row![
                    button(text("Cylinder").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Cylinder".to_string())),
                    button(text("Torus").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Torus".to_string())),
                ]
                .spacing(4),
                row![
                    button(text("Cone").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Cone".to_string())),
                    button(text("Plane").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Plane".to_string())),
                ]
                .spacing(4),
                row![
                    button(text("Teapot").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Teapot".to_string())),
                    button(text("Grid").size(11))
                        .padding([3, 6])
                        .on_press(OxideUiMessage::CreateDccPrimitive("Grid".to_string())),
                ]
                .spacing(4),
            ]
            .spacing(6),
            CommandPanelTab::Modify => {
                let stack_col = app.command_panel.modify.stack.iter().fold(
                    column![].spacing(3),
                    |col, item| {
                        col.push(
                            row![
    ...
    let center_view = row![viewport, command_panel]
        .width(Length::Fill)
        .height(Length::Fill);
    ...
    let timeline_bar = row![
        button(
            text(if app.animation_system.is_playing {
                "Pause ⏸"
            } else {
                "Play ▶"
            })
        ...
        text(format!(
            "Frame: {} / {}",
            app.animation_system.current_frame, app.animation_system.end_frame
        ))
        .size(11),
        text("│ Shading: Realistic (F3/F4) │").size(11),
        button(
            text(if app.dcc_viewport.is_maximized {
                "Restore View"
            } else {
                "Maximize (Alt+W)"
            })
    ...
    let mut main_col = column![container(menu_bar_row).padding(4),];
