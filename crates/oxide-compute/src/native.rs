...
impl NativeComputeDevice {
    ...
    pub fn new(backend: BackendKind, name: &str, total_mem: u64, is_unified: bool) -> Self {
        let capabilities = DeviceCapabilities {
            ...
            supports_fp64: true,
            supports_fp16: true,
            supports_tensor_cores: matches!(
                backend,
                BackendKind::Cuda | BackendKind::RocmHip | BackendKind::Metal
            ),
...
impl ComputeDevice for NativeComputeDevice {
    ...
    fn free_buffer(&self, buffer: BufferId) -> Result<(), ComputeError> {
        ...
        if map.remove(&buffer).is_some() {
            ...
            Err(ComputeError::InvalidHandle(format!(
                "Buffer {:?} not found",
                buffer
            )))
        }
    ...
    fn compile_kernel(&self, name: &str, source: &str) -> Result<KernelId, ComputeError> {
        ...
        let mut map = self
            .kernels
            .write()
            .map_err(|e| ComputeError::KernelCompilation(e.to_string()))?;
    ...
    fn dispatch(
        ...
        // Validate kernel existence
        {
            ...
            if !k_map.contains_key(&kernel) {
                return Err(ComputeError::InvalidHandle(format!(
                    "Kernel {:?} not compiled",
                    kernel
                )));
            }
...
mod tests {
    ...
    fn test_native_compute_device_lifecycle() {
        let dev = NativeComputeDevice::new(
            BackendKind::Cuda,
            "NVIDIA RTX 4090",
            24 * 1024 * 1024 * 1024,
            false,
        ...
        // Allocate buffer
        let buf = dev
            .allocate_buffer(1024, BufferUsage::Storage)
            .expect("Failed to allocate buffer");

        // Write data
        let input_data = vec![42u8; 128];
        dev.write_buffer(buf, 0, &input_data)
            .expect("Failed to write buffer");

        // Read data back
        let mut read_back = vec![0u8; 128];
        dev.read_buffer(buf, 0, &mut read_back)
            .expect("Failed to read buffer");
        assert_eq!(input_data, read_back);

        // Compile and dispatch kernel
        let kernel = dev
            .compile_kernel("vector_add", "__global__ void add() {}")
            .expect("Failed to compile kernel");
        dev.dispatch(kernel, [1, 1, 1], [32, 1, 1], &[buf])
