...
use crate::boolean::{BooleanOptions, boolean_op};
use crate::feature_ops::{ExtrudeOptions, FilletOptions, extrude_face};
...
impl std::fmt::Debug for NativeGeometryKernel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NativeGeometryKernel")
...
impl GeometryKernel for NativeGeometryKernel {
    ...
    fn extrude(
        ...
        let mut db = self
            ...
            .lock()
            .map_err(|e| KernelError::Internal(e.to_string()))?;
        extrude_face(&mut db, profile, direction, opts).ok_or_else(|| {
            KernelError::Internal("Extrusion failed: invalid profile or topology".to_string())
        })
    }
    ...
    fn fillet(
        ...
        if _opts.radius <= 0.0 {
            return Err(KernelError::InvalidFilletRadius(
