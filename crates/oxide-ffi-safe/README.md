...
> FFI-safe C/C++ ABI boundary wrappers for external bindings.

## Overview
`oxide-ffi-safe` is an integral crate in the **Oxide-3D** industrial CAD/CAE/CAM/DCC ecosystem. 

Zero-overhead ABI-safe wrappers for interoperability with external solvers, C APIs, and foreign language bindings.
...
```toml
[dependencies]
oxide-ffi-safe = { path = "../../crates/oxide-ffi-safe" }
```
