...
> WebGPU (wgpu) portable compute backend.

## Overview
`oxide-backend-wgpu` is an integral crate in the **Oxide-3D** industrial CAD/CAE/CAM/DCC ecosystem. 

Modern portable compute shader execution across desktop and WebAssembly (Wasm) targets.
...
```toml
[dependencies]
oxide-backend-wgpu = { path = "../../crates/oxide-backend-wgpu" }
```
